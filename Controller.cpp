...
namespace violet {
	void RambleController::update() {
		...
		if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS) {
			...
			cout << (newy - _cursorY) * _deltaTime << " " << (newx - _cursorX) * _deltaTime << endl;
			//_owner->rotate(vec3{ (newy - _cursorY) * _deltaTime * _rotateSpeed,(newx - _cursorX) * _deltaTime * _rotateSpeed,0.f});
			_cursorX = newx;
			_cursorY = newy;
		}
